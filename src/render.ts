...
const HEADER = '<!-- GENERATED by agent-docs — DO NOT EDIT BY HAND. Regenerate with `agent-docs`. -->'
...
export function buildFiles(rows: Row[], meta: Meta): Map<string, string> {
  ...
  // Agent-readable surface (llms.txt spec) — type-derived, so it is deterministic
  // and gated alongside the rest. This is the wedge: an llms.txt that can't rot
  // against the real exports because `--check` fails when it diverges.
...
function purpose(meta: Meta): string {
  return meta.description ?? `Deterministic export surface for ${meta.title}.`
}

/** llms.txt — curated agent-readable index: H1 + purpose + a link per entry. */
function renderLlmsTxt(rows: Row[], meta: Meta): string {
  ...
  for (const { entry, exports, error } of rows) {
    ...
    const names =
      error || !exports.length
        ? ''
        : ` — ${exports.slice(0, 8).map((e) => e.name).join(', ')}${exports.length > 8 ? ', …' : ''}`
    lines.push(`- [\`${entry.exportPath}\`](api/${entry.apiName}.md): ${count}${names}`)
  }
  return `${lines.join('\n').trimEnd()}\n`
}

/** llms-full.txt — the whole surface inlined: every export, kind, signature, doc. */
function renderLlmsFull(rows: Row[], meta: Meta): string {
  ...
  for (const { entry, exports, error, deps } of rows) {
    ...
    lines.push(`Source: \`${entry.rel}\`${deps.length ? ` · depends on ${deps.map((d) => `\`${d}\``).join(', ')}` : ''}`, '')
    if (error) {
